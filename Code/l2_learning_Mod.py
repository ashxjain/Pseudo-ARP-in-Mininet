# Copyright 2014 Ashish Jain

"""
A Modified L2 learning switch.
"""
...
class LearningSwitch (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    def checkIpMacTable(packet):
      ipdst = getDstIp(packet)
      return self.ipToMac.get(ipdst)

    #if self.macToPort.get(packet.src) is None:
    addIpMac(packet)
    self.macToPort[packet.src] = event.port # 1
    print event.port
    print "Port is %s and switch is %s" %(event.port,event.dpid)
    ...
    if packet.dst.is_multicast:
        dstMac = checkIpMacTable(packet)
        #log.debug(dstMac)
        #log.debug(self.ipToMac)
        if dstMac is not None:
          #log.debug("Testing")
          send_packet(packet,event,self.macToPort[dstMac])
        else:
          flood()
    else:
